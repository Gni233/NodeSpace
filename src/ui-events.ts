...
export function setupCanvasEvents(
  ...
  const showBoxMenu = (selNodes: any[], selEdges: { edge: any; idx: number }[], clientX: number, clientY: number) => {
    ...
    if (selNodes.length > 0) {
      ...
      const batchColorItem: any = { label: '批量颜色', keepOpen: true, action: () => {
        ...
        PRESET_COLORS.forEach(c => {
          ...
          sw.style.cssText = `width:20px;height:20px;background:${c};border-radius:4px;cursor:pointer;border:1px solid rgba(255,255,255,0.2);`;
          sw.onclick = () => {
            for (const id of ids) {
              const n = ctx.graph.nodes.find((n: any) => n.id === id);
              if (n) n.color = c;
    ...
    if (selEdges.length > 0) {
      items.push({ label: `删除连线 (${selEdges.length})`, action: () => {
        ...
        ctx.graph.edges = ctx.graph.edges.filter((_: any, i: number) => !edgeIdxSet.has(i));
        selectedNodeIds = [];
        ctx.triggerSave?.();
        ctx.initSim?.();
        ctx.clearEd?.();
  ...
  canvas.addEventListener("touchmove", (e: TouchEvent) => {
    ...
    if (hoverNode?.mediaType) ctx.onMediaHover?.(hoverNode.id);
    else ctx.onMediaHover?.(null);
  ...
  canvas.addEventListener("pointerup", (e: PointerEvent) => {
    if (e.button === 2) {
      // 右键框选结束（右键拖拽连线已移至 contextmenu 兜底）
  ...
  canvas.addEventListener("contextmenu", (e: MouseEvent) => {
    ...
    // suppressContextMenu 标志：pointerdown 已命中节点，不弹菜单
