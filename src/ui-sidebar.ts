...
export function createSidebar(
  ...
  title.style.cssText = `font-weight:700;font-size:${V('--fg-font-lg', '0.95em')};color:${V('--fg-text', '#e0e0e0')};letter-spacing:0.02em;`;
  ...
  newFileBtn.onclick = async () => {
    const name = await safePrompt('输入图名：');
  ...
  return { sidebar, updateFileTree, getCurrentFile: () => currentFile, syncActiveFile: (name: string) => { currentFile = name; if (treeData.length > 0) updateFileTree(treeData, name); } };
}
