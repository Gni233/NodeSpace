...
export { resolveSemanticZoomBand as resolveSemanticLensBand } from './semantic-zoom';

const semanticNodeBody = (node: any): string => node?.spaceRef
  && typeof node?._spaceReferencePreview === 'string'
  ? node._spaceReferencePreview.trim()
  : node?.resourceRef && typeof node?._resourceReferencePreview === 'string'
    ? node._resourceReferencePreview.trim()
    : String(node?.note || '').trim();

export type SemanticLensBand = SemanticZoomBand;
...
function informationScore(node: any, degree: number, sourceIndex: number, total: number): number {
  ...
  const noteLength = Array.from(body).length;
  ...
  return Math.min(1, noteLength / 220) * 0.34
    ...
    + recency * 0.07
    + (node.fixed ? 0.16 : 0)
    + (node.resourceRef ? 0.18 : node.mediaType ? 0.08 : 0)
    + actionable;
}
...
export function computeSemanticLens(options: SemanticLensOptions): SemanticLensDecision {
  ...
  const focusNodeId = options.focusNodeId && alive.has(String(options.focusNodeId))
    ...
    : null;
  const band = resolveSemanticZoomBand(options.zoom ?? 1, options.previous?.band);
  const informationBudget = semanticInformationBudget({
    band,
  ...
  const budget = informationBudget.cardCount;
  ...
  return {
    band,
    budget,
    edgeLabelBudget: informationBudget.edgeLabelCount,
    regionLabelBudget: informationBudget.regionLabelCount,
    echoBudget: informationBudget.echoCount,
    focusNodeId,
    expandedNodeIds,
