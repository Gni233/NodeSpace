import {Z_TOAST, Z_MODAL_BACKDROP, TOAST_MAX_COUNT, V } from "./layout-constants";
...
export function showToast(msg: string, type: 'info' | 'success' | 'error' | 'warning' = 'info', duration = 2500): void {
  ...
  toast.style.cssText =
    `padding:7px 16px;border-radius:${V('--fg-radius-md', '10px')};` +
...
export function confirmAction(
  ...
  return new Promise((resolve) => {
    ...
    dialog.style.cssText =
      ...
      `border:1px solid ${V('--fg-glass-border', 'rgba(255,255,255,0.1)')};` +
      `border-radius:${V('--fg-radius-lg', '14px')};padding:16px 20px;` +
